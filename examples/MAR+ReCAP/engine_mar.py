...
def evaluate(model_without_ddp, vae, ema_params, args, epoch, batch_size=16, log_writer=None, cfg=1.0,
    ...
    save_folder = os.path.join(args.output_dir, "ariter{}-diffsteps{}-temp{}-{}cfg{}-image{}-pre{}-cache{}-buffer{}".format(args.num_iter, args.num_sampling_steps,args.temperature,args.cfg_schedule,cfg, args.num_images, args.pre_full_iters, args.num_cache_iters, args.cache_buffer))
    if use_ema:
    ...
    if misc.get_rank() == 0:
        if not os.path.exists(save_folder):
            ...
            num_files = len(os.listdir(save_folder))
            if num_files == args.num_images:
                num_steps = 0
                print("Already generated {} images, skip generation".format(num_files))
    ...
    if log_writer is not None:
        if args.img_size == 256:
            ...
            with np.load(fid_statistics_file) as f:
                m2, s2 = f['mu'][:], f['sigma'][:]
                m2, s2 = torch.from_numpy(m2).cuda(), torch.from_numpy(s2).cuda()
        else:
        ...
        with open(os.path.join(args.output_dir, 'res.txt'), 'a') as f:
            f.write("ariter{}-temp{}-{}cfg{}-image{}-pre{}-cache{}-buffer{}-fast{} FID: {:.4f}, Inception Score: {:.4f} Total Time: {:.5f} Diff Time:{:.5f})\n".format(args.num_iter,args.temperature,args.cfg_schedule,cfg, args.num_images, args.pre_full_iters, args.num_cache_iters, args.cache_buffer, args.diff_fast_ratio, fid, inception_score, used_time/gen_img_cnt, used_diff_time/gen_img_cnt))
